		...
		static void Main (string[] args) {
			...
			for (int i = 0; i < numbers.Length; i++) {
				for (int j = 0; j < numbers.Length; j++) {
					if (i == j)
						continue;
					maxMagnitude = Math.Max (Magnitude (Add (numbers[i].Copy (), numbers[j].Copy ())), maxMagnitude);
					maxMagnitude = Math.Max (Magnitude (Add (numbers[j].Copy (), numbers[i].Copy ())), maxMagnitude);
				}
			}
