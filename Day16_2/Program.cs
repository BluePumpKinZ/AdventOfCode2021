	...
	class Program {
		...
		class Packet {
			...
			public long GetValue () {
				...
				switch (ID) {
				case 0:
					toReturn = SubPackets.Sum (t => t.GetValue ());
					break;
				case 1:
					long total = 1;
					for (int i = 0; i < SubPackets.Count; i++) {
						total *= SubPackets[i].GetValue ();
					}
					toReturn = total;
					break;
				case 2:
					toReturn = SubPackets.Min (t => t.GetValue ());
					break;
				case 3:
					toReturn = SubPackets.Max (t => t.GetValue ());
					break;
				case 4:
					toReturn = Value;
					break;
				case 5:
					toReturn = SubPackets[0].GetValue () > SubPackets[1].GetValue () ? 1 : 0;
					break;
				case 6:
					toReturn = SubPackets[0].GetValue () < SubPackets[1].GetValue () ? 1 : 0;
					break;
		...
		static void Main (string[] args) {
			...
			int index = 0;
			Packet packet = GetPacketFromArray (bits, ref index);
			Console.WriteLine (packet.GetValue ());
		}

		static Packet GetPacketFromArray (BitArray array, ref int index) {
			...
			default:
				...
				if (!array[index++]) {
					...
					while (index - startIndex < length - 1) {
						toReturn.SubPackets.Add (GetPacketFromArray (array, ref index));
					}
				} else {
					// L1
